...
- PostgreSQL migration guide (step-by-step diff)
...
```
Everything else in the image upload pipeline is ready (Multer, Cloudinary SDK, upload route, controller).

### 8. GitHub repository
**Status**: ❌ User will do  
Push `buddyscript/` to GitHub with clean commit history.

### 9. Live deployment
**Status**: ❌ User will do  
- Frontend → Vercel (set root to `client/`)
- Backend → Railway (set root to `server/`, add PostgreSQL plugin)
...
| README.md | ✅ Complete |
| Image upload (Cloudinary) | ⚠️ Needs credentials |
| GitHub repo | ⚠️ User will push |
